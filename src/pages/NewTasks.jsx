...
const NewTasks = () => {
    ...
    const fetchNewTasks = async () => {
      try {
        const response = await axios.get(
          `${backend_url}/api/get-new-tasks?page=${page}&limit=5`,
  ...
  const handleStatusUpdate = async (taskId, newStatus) => {
    try {
      await axios.put(
        `${backend_url}/api/update-task-status/${taskId}`,
        { status: newStatus },
        { headers: { Authorization: token } }
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-2 py-2">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">New Tasks</h2>

        {loading ? (
          <p className="text-gray-500 text-center py-4">Loading tasks...</p>
        ) : tasks.length === 0 ? (
          <p className="text-gray-500 text-center py-4">No new tasks assigned.</p>
        ) : (
            ...
            {tasks.map((task) => (
              ...
              >
                ...
                <div className="flex gap-3 mt-2">
                  <button
                    onClick={() => handleStatusUpdate(task.task_id, "in-progress")}
                    className="bg-yellow-400 text-white px-3 py-1 rounded-lg hover:bg-yellow-500 transition"
                  >
                    Mark In-Progress
                  </button>
                  <button
                    onClick={() => handleStatusUpdate(task.task_id, "done")}
